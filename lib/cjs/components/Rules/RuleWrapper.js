...
var RuleWrapper = (0, react_1.memo)(function (_a) {
    ...
    var trimmedProperty = data.property.trim();
    var trimmedValue = data.value.trim();
    var handleBlur = function () {
        if (!trimmedProperty)
            onRemove(data.id);
    };
    var isValid = trimmedValue ? CSS.supports(trimmedProperty, trimmedValue) : true;
    return (react_1.default.createElement(StyledWrapper, { onBlur: handleBlur, backgroundColor: theme.background, shadow: theme.shadow, radius: theme.radius, spacing: theme.spacing },
        ...
        trimmedProperty && (react_1.default.createElement(RuleValueWrapper_1.default, { value: data.value, mode: defaultMode, onChange: handleChangeValue, property: data.property, valueRef: valueRef, order: order + 1, isValid: isValid }))));
});
