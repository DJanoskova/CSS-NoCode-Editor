...
var CSSBuilder = function (_a) {
    ...
    var _d = useState(function () { return calculateStyleArray(style); }), localStyle = _d[0], setLocalStyle = _d[1];
    var stringifiedTheme = JSON.stringify(theme);
    var combinedTheme = useMemo(function () {
        return __assign(__assign({}, defaultTheme), JSON.parse(stringifiedTheme));
    }, [stringifiedTheme]);
    ...
    return (React.createElement(ThemeContext.Provider, { value: combinedTheme },
        React.createElement(StyledWrapper, { fontSize: combinedTheme.fontSize, color: combinedTheme.color }, localStyle.map(function (rule, index) {
